...
export namespace useProcurementAtasServiceParams {
  export type Get = {
    item: LicitacaoItem
    enabled?: boolean
  }
...
export function useProcurementAtasService(api: CoreApiClient) {
  const get = (params: useProcurementAtasServiceParams.Get) => {
    const { item, enabled = true } = params
    const canFetch = enabled && !!item.orgao_cnpj && !!item.ano && !!item.numero_sequencial

    return useQuery({
      queryKey: ["procurement-atas", item.orgao_cnpj, item.ano, item.numero_sequencial],
      queryFn:  () => api.search.fetchExternalProcurementAtas({
