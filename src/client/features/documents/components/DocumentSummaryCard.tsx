...
import type { GenerateDocumentSummaryResponse } from "@/client/features/licitacoes/services/use-document-summary.service"

type Props = {
  summary: GenerateDocumentSummaryResponse
}

export function DocumentSummaryCard({ summary }: Props) {
  return (
    <div className="min-w-0 space-y-4 rounded-[1.2rem] border border-slate-200/80 bg-white p-4 shadow-[0_10px_30px_rgba(15,23,42,0.04)]">
      ...
      <section className="space-y-2">
        <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-slate-500">Visao geral</p>
        <p className="whitespace-pre-wrap break-words text-sm leading-6 text-slate-700 [overflow-wrap:anywhere]">
          {summary.summary.overview}
        </p>
      </section>

      <SummaryList icon={FileText} title="Principais pontos" items={summary.summary.keyPoints} />
      <SummaryList icon={TimerReset} title="Prazos e datas criticas" items={summary.summary.deadlines} />
      <SummaryList icon={FileText} title="Requisitos relevantes" items={summary.summary.requirements} />
      <SummaryList icon={ShieldAlert} title="Pontos de atencao" items={summary.summary.risks} />
